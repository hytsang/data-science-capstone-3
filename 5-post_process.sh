...
grep -h "^[a-zA-Z]\+-\?[a-z]*|[a-zA-Z]\+-\?[a-z]*|[0-9]*" $GRAMS_DIR/bigrams-*.csv > $GRAMS_DIR/bigrams.unaggregated.valid.csv
grep -h "^[a-zA-Z]\+-\?[a-z]* n\?'[a-z]\+|[a-zA-Z]\+-\?[a-z]*|" $GRAMS_DIR/trigrams-*.csv | sed 's/ //g' >> $GRAMS_DIR/bigrams.unaggregated.valid.csv
sort -k 3 -t '|' -nr $GRAMS_DIR/bigrams.unaggregated.valid.csv > $GRAMS_DIR/bigrams.unaggregated.valid.sorted.csv
...
grep -h "^[a-zA-Z]\+-\?[a-z]* [a-zA-Z]\+-\?[a-z]*|n\?'[a-z]\+|" $GRAMS_DIR/trigrams-*.csv | sort -k 3 -t '|' -nr > $GRAMS_DIR/bigrams.unaggregated.contractions.sorted.csv
